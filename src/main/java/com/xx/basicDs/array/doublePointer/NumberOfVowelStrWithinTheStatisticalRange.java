package com.xx.basicDs.array.doublePointer;
...
public class NumberOfVowelStrWithinTheStatisticalRange implements Answer {
    ...
    public int vowelStrings(String[] words, int left, int right) {
        ...
        while (left <= right) {
            if (left == right) {
                ...
                String wordL = words[left];
                String wordR = words[right];
                if (vowel.contains(wordL.charAt(0)) && vowel.contains(wordL.charAt(wordL.length() - 1))) {
                    count++;
                }
                if (vowel.contains(wordR.charAt(0)) && vowel.contains(wordR.charAt(wordR.length() - 1))) {
                    count++;
                }
            }
            left++;
    ...
    public String[] initData() {
        return new String[]{"hey", "aeo", "mu", "ooo", "artro", "gdf"};
    }
}